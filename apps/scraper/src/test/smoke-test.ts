...
describe('Smoke Tests - Database Operations', () => {
  ...
  describe('Jobs Operations', () => {
    ...
    it('should insert and select a job', async () => {
      ...
      const inserted = await queryOne<{ id: number }>(
        `SELECT id FROM jobs WHERE type = 'test-job' ORDER BY id DESC LIMIT 1`
      );
  ...
  describe('Scrape Runs Operations', () => {
    it('should insert a scrape run with correct columns', async () => {
      ...
      const inserted = await queryOne<{ id: string }>(
        `SELECT id FROM scrape_runs WHERE site_id = $1 ORDER BY created_at DESC LIMIT 1`,
        [site.id]
      );
      assert.ok(inserted?.id);
      await query('DELETE FROM scrape_runs WHERE id = $1', [inserted!.id]);
    });
  });
  ...
  describe('SSE Events Operations', () => {
    it('should insert sse_event with event_channel (not channel)', async () => {
      ...
      `);
      const inserted = await queryOne<{ id: number }>(
        `SELECT id FROM sse_events WHERE event_type = 'test-event' ORDER BY id DESC LIMIT 1`
      );
      assert.ok(inserted?.id);
      await query('DELETE FROM sse_events WHERE id = $1', [inserted!.id]);
    });
  });
