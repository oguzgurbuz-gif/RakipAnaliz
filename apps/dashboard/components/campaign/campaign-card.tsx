...
import { formatDate, formatDateRange, getSentimentColor, getStatusColor, cn } from '@/lib/utils'
        ...
        <CardContent className="space-y-2">
          {(campaign.metadata as any)?.ai_analysis?.campaign_type ? (
            <div className="text-sm">
              <span className="font-medium">Tür:</span> {(campaign.metadata as any)?.ai_analysis?.campaign_type}
            </div>
          ) : campaign.category && (
            <div className="text-sm">
              <span className="font-medium">Kategori:</span> {campaign.category}
            </div>
          )}
          ...
          <div className="text-xs text-muted-foreground">
            ...
            {(campaign.validFrom || campaign.validTo) && (
              <div>Geçerlilik: {formatDateRange(campaign.validFrom, campaign.validTo)}</div>
            )}
          </div>
